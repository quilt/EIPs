...
`AUTHCALL` is interpreted the same as `CALL`, except for:
 ...
 - If the gas available for subcall would be less than `gas`, execution is invalid.
...
`AUTH` allows for signatures to be signed by `tx.origin`. For any such signatures, subsequent `AUTHCALL`s have `msg.sender == tx.origin` in their first layer of execution. Currently, this situation can only ever arise in the topmost execution layer of a transaction. This EIP breaks that invariant and thus affects smart contracts containing `require(msg.sender == tx.origin)` checks. This check is used for either of two purposes:

 - Ensuring that `msg.sender` is an EOA (given that `tx.origin` alway has to be an EOA). This invariant does not depend on the execution layer depth and thus is not affected.
 - Protecting against atomic sandwich attacks like flash loans, that rely on the ability to modify state before and after the execution of the target contract as part of the same atomic transaction. This protection would be broken by this EIP. However, it is considered bad pracice and has already been mostly made obsolete with the recent emergence of flash bots that enable non-atomic sandwiching attacks with capabilities approaching those of their atomic counterparts.

Both of these use cases can be found in contracts deployed on Ethereum mainnet, with the first (and unaffected) one being more common. In addition to these two, there is a third use case of checking `msg.sender == tx.origin` for re-entrancy protection, again via the topmost execution layer guarantee. This one is not actually known to be used, but if existing would be more severly affected. However, not only did the authors of this EIP not find any examples for such a form of re-entrancy protection, the absense of such examples is exactly what one would expect:

 - There are few other fully effective ways to check for `msg.sender` being an EOA, so seeing it used for that is expected.
 - There are no other ways to protect against atomic sandwich attacks when called from an untrusted `msg.sender`, so seeing it used for that is expected.
 - In contrast, there are many more direct and more flexible ways to guarantee re-entrancy protection. Given that `msg.sender == tx.origin` only happens to have the property of only being true only on the topmost execution layer, it is not to be expected that people would choose to rely on it for an obscure version of such protection.


The reason for allowing signatures from `tx.origin` is to enable use cases like simple transaction batching, where the sender of the outer transaction in the simplest case would be identical to the signing account. It would possible to otherwise mitigate this restriction, including via the following approaches:
...
* Completely ban `tx.origin` as signer. This would make the simple batching use cases impossible, but could be later on relaxed via one of the other approaches.
