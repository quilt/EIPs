...
`AUTHCALL` is interpreted the same as `CALL`, except for:
 ...
 - The gas cost and the gas available in the subcall is specified in the Gas Cost section.
 - If the gas available in the subcall is less than `gas`, execution is invalid.
...
The gas cost for `AUTHCALL` shall be the **sum** of:
 ...
 - gas available for execution in the subcall (`target_gas`)

The memory expansion gas cost (`memory_expansion_fee`) shall be calculated in the same way as `CALL`.

The dynamic gas portion (`dynamic_gas`), and the gas available for execution in the subcall (`target_gas`) shall be calculated as:
