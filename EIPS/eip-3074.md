...
`AUTHCALL` is interpreted the same as `CALL`, except for:
 ...
 - If `authorized` is unset, execution is invalid. Otherwise, the caller address for the call is set to `authorized`.
 - If less than `gas` gas is available for the call (see Gas Cost section), execution is invalid. If `gas` is `0`, all available gas is passed into the call. Otherwise, `gas` is passed.
...
After deducting the opcode gas cost from the total available gas, the gas available for the call is calculated as `available_gas - (available_gas // 64)` analogous to EIP-150. The gas to be passed into the call is calculated as specified in the Behavior section. As with `CALL`, it is charged immediately, independent of whether the call is actually executed.
