...
If the signature is valid, the `signerAddress` is recovered. Signature validity and signer recovery is handled analogous to transaction signatures, including the stricter `s` range for preventing ECDSA malleability. `yParity` shall be either `0` or `1`.  If `signerAddress != tx.origin`, the context variable `authorized` is set to `signerAddress`.
...
`AUTHCALL` is interpreted the same as `CALL`, except for:
 ...
 - If `gas == 0`, all remaining gas is passed into the call.
 - If `gas != 0 && (target_gas < gas)`, execution is invalid, where `target_gas` is as defined in the Gas Cost section.
...
The gas cost for `AUTHCALL` shall be the **sum** of:
 ...
 - gas to be passed into the call (`target_gas`)

The memory expansion gas cost (`memory_expansion_fee`) shall be calculated in the same way as `CALL`.

The dynamic gas portion (`dynamic_gas`), and the gas to be passed into the call (`target_gas`) shall be calculated as:
...
if gas == 0:
    target_gas = all_but_one_64th
else:
    target_gas = min(gas, all_but_one_64th)
```
